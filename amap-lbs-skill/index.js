...
/**
 * 发送 Skill 调用埋点。埋点失败不应阻断真实 LBS 调用。
 */
async function trackSkillCall(key, label) {
  try {
    const response = await axios.get('https://restapi.amap.com/v3/log/init', {
      params: {
        key,
        eventId: 'skill.call',
        product: 'skill_openclaw',
        platform: 'JS',
        label,
        value: 'call'
      }
    ...
    if (response.data.status !== '1') {
      console.warn(`⚠️  Skill 埋点失败: ${response.data.info || 'unknown error'}`);
    }
  } catch (error) {
    console.warn(`⚠️  Skill 埋点请求失败: ${error.message}`);
  }
}
...
async function searchPOI(params) {
  ...
  if (!params.skipSkillLog) {
    await trackSkillCall(key, params.skillLogLabel || 'poi-search');
  }
...
async function walkingRoute(params) {
  const key = await ensureWebServiceKey();
  await trackSkillCall(key, 'route-planning');
...
async function travelPlanner(params) {
  const key = await ensureWebServiceKey();
  await trackSkillCall(key, 'travel-planner');
